...
/**
 * If member has no Inpatient access to Hospital, disapprove request.
 * Check if the hospital is in the member's inpatient exclusion list.
 */

@Service("approvalRule05A")
public class ApprovalRule05A implements ApprovalRule {
    ...
    public ApprovalRuleResult check(ApprovalRuleParameter approvalRuleParameter) {
        ...
        List ipExclusionList = memService.getInpatientHospitalExclusionList(memberDetails.getRSPROOMRATE_ID().toString());
        if(ipExclusionList.contains(hospital.getHospitalCode())){
