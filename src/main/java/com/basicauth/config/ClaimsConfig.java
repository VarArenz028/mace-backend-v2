...
public class ClaimsConfig {
    ...
    private static Logger logger = LoggerFactory.getLogger(ClaimsConfig.class);

    @Bean
    @ConfigurationProperties(prefix = "claims.jdbc")
    public DataSource dataSourceClaims() {
        logger.info("dataSourceClaims");
...
//        return ds;
    ...
    @Bean
    public SqlSessionFactory sqlSessionFactoryClaims() throws Exception {
...
//				this.getClass().getResourceAsStream("classpath:mybatis-config.xml"));
        ...
        sqlSessionFactory.setDataSource(dataSourceClaims());
        sqlSessionFactory.setConfigLocation(new ClassPathResource("mybatis-config.xml"));
        sqlSessionFactory.setFailFast(true);
        Resource mapperResource00 = new ClassPathResource("mapper/ClaimsMapper.xml");
...
//        sqlSessionFactory.setTypeHandlersPackage("org.horiga.study.mybatis.typehandler");
    ...
    public DataSourceTransactionManager transactionManagerClaims() {
        ...
        return new DataSourceTransactionManager(dataSourceClaims());
    }

    @Bean
    ClaimsMapper claimsMapper() throws Exception {
        logger.info("claimsMapper");
        SqlSessionTemplate sessionTemplate = new SqlSessionTemplate(sqlSessionFactoryClaims());
        return sessionTemplate.getMapper(ClaimsMapper.class);
    }
}
